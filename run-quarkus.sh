...
./Maven/apache-maven-3.5.3/bin/mvn clean install -Dquarkus "${NATIVE}" -DJBOSS_VERSION=${JBOSS_VERSION} -pl quarkus-documentation -Dquarkus-project-discovery=false -Dquarkus-cp-cache=false -Dquarkus-workspace-discovery=false -Dquarkus-classpath-cache=false
