...
public class HATimerService implements Service<String> {
    ...
    public void start(StartContext arg0) throws StartException {
        ...
        LOGGER.info("Start HASingleton timer service 1 '" + this.getClass().getName() + "'");

        final String node = System.getProperty("jboss.node.name");
        try {
            InitialContext ic = new InitialContext();
        //    ((Scheduler) ic.lookup("global/testClusterDublicateServiceException-ear-1.0.3.Final-SNAPSHOT/testClusterDublicateServiceException-ejb-1.0.3.Final-SNAPSHOT/SchedulerBean!org.jboss.metrics.Scheduler")).initialize("HASingleton timer @" + node + " " + new Date());
    ...
    public void stop(StopContext arg0) {
        if (!started.compareAndSet(true, false)) {
            ...
            LOGGER.info("Stop HASingleton timer service 1 '" + this.getClass().getName() + "'");
            try {
                InitialContext ic = new InitialContext();
           //     ((Scheduler) ic.lookup("global/testClusterDublicateServiceException-ear-1.0.3.Final-SNAPSHOT/testClusterDublicateServiceException-ejb-1.0.3.Final-SNAPSHOT/SchedulerBean!org.jboss.metrics.Scheduler")).stop();
            } catch (NamingException e) {
